...
class Window_Add_User():
    ...
    def __init__(self):
        ...
        self.window.title("상세 회원 정보")
        self.window.resizable(width=FALSE, height=FALSE)
        def func_exit(event):
            self.window.quit()
            self.window.destroy()
        self.user_editor = Window_Edit_User(self.window, x=140, y=10)   # 회원 Edit 패널을 윈도우에 포함시킴
    ...
    def add_user(self):
        ...
        user_id = self.user_editor.get_id()
        user_password = self.user_editor.get_password()
        address = "sample_image/"+user_phone+".png"
        new_user = pd.DataFrame.from_dict([{ "USER_PHONE": user_phone, "USER_ID":user_id, "USER_PW":user_password ,"USER_NAME": user_name,"USER_BIRTH": user_birthday,
...
class Window_Add_Book():
    ...
    def __init__(self):
        ...
        def func_exit(event):
            self.window.quit()
            self.window.destroy()
        self.book_editor = Window_Edit_Book(self.window, x=140, y=10)   # 도서 Edit 패널을 윈도우에 포함시킴
